...
<template>
  <MainLayout>
    ...
    <Teleport to="body"><div v-if="showQuotationModal" class="fixed inset-0 z-[120] flex items-center justify-center bg-black bg-opacity-70 backdrop-blur-sm p-4"><div class="bg-white rounded-xl shadow-2xl w-full max-w-lg relative animate-fade-in-up"><div class="bg-white p-5 border-b rounded-t-xl flex justify-between items-center"><h3 class="font-bold text-gray-800 text-lg flex items-center gap-2"><span class="w-2 h-6 bg-purple-500 rounded-full inline-block"></span> Input Quotation</h3><button @click="cancelMoveQuotation" class="text-gray-400 hover:text-gray-600"><X :size="24"/></button></div><div class="p-6"><form @submit.prevent="saveQuotationInitial" class="space-y-4"><div class="bg-purple-50 p-3 rounded text-sm text-purple-700 mb-4"><strong>Property:</strong> {{ draggedItem?.property }}</div><div><label class="label">PIC GP</label><select v-model="formQuotationInitial.pic_gp" class="input"><option v-for="pic in picList" :key="pic" :value="pic">{{ pic }}</option></select></div><div><label class="label">Date</label><input v-model="formQuotationInitial.date" type="date" class="input" required></div><div><label class="label">Link Quotation</label><div class="relative"><input v-model="formQuotationInitial.link_quotation" type="text" class="input" placeholder="google.com" @blur="autoFixUrl(formQuotationInitial, 'link_quotation')" required></div></div><div class="flex items-center gap-2 cursor-pointer" @click="formQuotationInitial.is_send = !formQuotationInitial.is_send"><CheckSquare :size="20" class="text-green-600" v-if="formQuotationInitial.is_send"/><Square :size="20" class="text-gray-400" v-else/><span class="text-sm font-medium text-gray-700">Sudah Terkirim?</span></div><div class="pt-4 flex justify-end gap-3 border-t mt-4"><button type="button" @click="cancelMoveQuotation" class="px-5 py-2.5 rounded-lg border bg-white hover:bg-gray-50 font-medium text-gray-700">Batal</button><button type="submit" class="px-5 py-2.5 rounded-lg bg-purple-600 text-white hover:bg-purple-700 font-bold shadow-lg flex items-center gap-2"><Save :size="18"/> Simpan</button></div></form></div></div></div></Teleport>
    <Teleport to="body"><div v-if="showActivityModal && selectedActivityLead" class="fixed inset-0 z-[110] flex items-center justify-center bg-black bg-opacity-60 backdrop-blur-sm p-4"><div class="bg-white rounded-xl shadow-2xl w-full max-w-7xl h-[85vh] flex flex-col overflow-hidden relative animate-fade-in-up"><div class="bg-slate-50 p-5 border-b shrink-0"><div class="flex justify-between items-start mb-4"><div><h2 class="text-2xl font-bold text-gray-800">{{ selectedActivityLead.property }}</h2><div class="flex items-center gap-2 mt-2"><span class="text-xs bg-gray-200 text-gray-700 px-2 py-1 rounded font-bold">{{ selectedActivityLead.source }}</span><span class="text-xs border border-gray-300 text-gray-600 px-2 py-1 rounded">{{ selectedActivityLead.type }}</span></div></div><button @click="showActivityModal = false" class="text-gray-400 hover:text-red-500 bg-white p-1.5 rounded-full shadow-sm transition"><X :size="24"/></button></div><div class="grid grid-cols-4 gap-6 text-sm text-gray-600 bg-white p-4 rounded-lg border border-gray-200 shadow-sm mb-3"><div><span class="block font-bold text-gray-400 uppercase text-[10px] mb-1">Alamat</span><span class="flex items-start gap-1 font-medium"><MapPin :size="14" class="mt-0.5"/> {{ selectedActivityLead.address || '-' }}</span></div><div><span class="block font-bold text-gray-400 uppercase text-[10px] mb-1">Koordinat</span><span class="font-mono bg-gray-100 px-2 py-0.5 rounded">{{ selectedActivityLead.coordinates || '-' }}</span></div><div><span class="block font-bold text-gray-400 uppercase text-[10px] mb-1">PIC GuestPro</span><span class="flex items-center gap-1 font-medium"><User :size="14"/> {{ selectedActivityLead.gp_pic }}</span></div><div><span class="block font-bold text-gray-400 uppercase text-[10px] mb-1">Tgl Masuk</span><span class="flex items-center gap-1 font-medium"><CalendarIcon :size="14"/> {{ selectedActivityLead.date_in }}</span></div></div><div class="bg-blue-50 border border-blue-100 rounded-lg overflow-hidden transition-all duration-300"><div @click="showPicList = !showPicList" class="px-4 py-2 flex justify-between items-center cursor-pointer hover:bg-blue-100 transition select-none"><span class="text-xs font-bold text-blue-700 uppercase flex items-center gap-2"><User :size="14" /> Daftar PIC Lead ({{ selectedActivityLead.pics.length }})</span><ChevronDown :size="16" class="text-blue-500 transition-transform duration-300" :class="showPicList ? 'rotate-180' : ''" /></div><div v-if="showPicList" class="border-t border-blue-100 bg-white p-3"><div v-if="selectedActivityLead.pics.length === 0" class="text-center text-xs text-gray-400 italic">Tidak ada data PIC.</div><div v-else class="grid grid-cols-2 md:grid-cols-3 gap-3"><div v-for="pic in selectedActivityLead.pics" :key="pic.id" class="text-xs border rounded p-2 bg-gray-50"><div class="font-bold text-gray-700">{{ pic.pic_name }}</div><div class="text-gray-500 mt-1 flex flex-col gap-0.5"><span v-if="pic.phone_number">📞 {{ pic.phone_number }}</span><span v-if="pic.whatsapp" class="text-green-600">💬 {{ pic.whatsapp }}</span><span v-if="pic.email" class="text-blue-500">✉️ {{ pic.email }}</span></div></div></div></div></div></div><div class="flex-1 flex flex-col bg-white overflow-hidden"><div class="flex justify-between items-center px-5 py-3 border-b bg-white"><div class="flex bg-gray-100 p-1 rounded-lg"><button @click="switchActivityTab('followup')" class="px-4 py-1.5 text-sm font-bold rounded-md transition-all" :class="activityTab==='followup' ? 'bg-white shadow text-blue-600' : 'text-gray-500 hover:text-gray-700'">Follow Up</button><button @click="switchActivityTab('meeting')" class="px-4 py-1.5 text-sm font-bold rounded-md transition-all" :class="activityTab==='meeting' ? 'bg-white shadow text-orange-600' : 'text-gray-500 hover:text-gray-700'">Meeting</button><button v-if="['quotation', 'deals', 'onboarding', 'retention'].includes(selectedActivityLead?.status_kanban)" @click="switchActivityTab('quotation')" class="px-4 py-1.5 text-sm font-bold rounded-md transition-all" :class="activityTab==='quotation' ? 'bg-white shadow text-purple-600' : 'text-gray-500 hover:text-gray-700'">Quotation</button></div><button @click="openInputModal(null)" class="bg-[#65a30d] hover:bg-[#4d7c0f] text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 shadow transition"><Plus :size="16" /> Tambah {{ activityTab === 'followup' ? 'Follow Up' : (activityTab === 'meeting' ? 'Meeting' : 'Quotation') }}</button></div><div class="flex-1 overflow-auto p-0"><table class="w-full text-left text-sm border-collapse"><thead class="bg-gray-50 text-gray-500 font-bold uppercase text-xs sticky top-0 z-10 shadow-sm"><tr><th class="p-4 border-b w-40">Tanggal & Waktu</th><th class="p-4 border-b w-32">PIC GP</th><th v-if="activityTab !== 'quotation'" class="p-4 border-b w-32">Tipe</th><th v-if="activityTab === 'meeting'" class="p-4 border-b w-40">Lokasi</th><th v-if="activityTab !== 'quotation'" class="p-4 border-b w-48">Objective</th><th v-if="activityTab !== 'quotation'" class="p-4 border-b">{{ activityTab === 'followup' ? 'Notes' : 'MOM' }}</th><th v-if="activityTab === 'quotation'" class="p-4 border-b">Link Document</th><th v-if="activityTab === 'quotation'" class="p-4 border-b w-24 text-center">Status</th><th class="p-4 border-b text-center w-24">Aksi</th></tr></thead><tbody class="divide-y divide-gray-100"><tr v-for="item in activityList" :key="item.id" class="hover:bg-gray-50 transition"><td class="p-4 align-top"><div class="font-bold text-gray-800">{{ item.date }}</div><div v-if="activityTab !== 'quotation'" class="text-xs text-gray-500 mt-1 flex items-center gap-1"><Clock :size="12"/> {{ item.start_time?.substring(0,5) }} - {{ item.end_time?.substring(0,5) }}</div></td><td class="p-4 align-top text-gray-700 font-medium">{{ item.pic_gp }}</td><td v-if="activityTab !== 'quotation'" class="p-4 align-top"><span class="px-2.5 py-1 rounded border text-xs font-semibold" :class="activityTab==='followup' ? 'bg-blue-50 text-blue-700 border-blue-100' : 'bg-orange-50 text-orange-700 border-orange-100'">{{ activityTab === 'followup' ? item.fu_type : item.meeting_type }}</span></td><td v-if="activityTab === 'meeting'" class="p-4 align-top text-gray-600">{{ item.location || '-' }}</td><td v-if="activityTab !== 'quotation'" class="p-4 align-top font-medium text-gray-700">{{ item.objective }}</td><td v-if="activityTab !== 'quotation'" class="p-4 align-top text-gray-600 whitespace-pre-line">{{ activityTab === 'followup' ? item.notes : item.mom }}</td><td v-if="activityTab === 'quotation'" class="p-4 align-top"><a :href="item.link_quotation" target="_blank" class="text-blue-600 hover:underline flex items-center gap-1"><LinkIcon :size="14" class="shrink-0"/> <span class="truncate max-w-[200px]">{{ item.link_quotation }}</span></a></td><td v-if="activityTab === 'quotation'" class="p-4 align-top text-center"><span class="px-2 py-1 rounded text-xs font-bold" :class="item.is_send ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'">{{ item.is_send ? 'Terkirim' : 'Draft' }}</span></td><td class="p-4 align-top text-center"><div class="flex justify-center gap-2"><button @click="openInputModal(item)" class="text-blue-500 hover:bg-blue-100 p-1.5 rounded transition" title="Edit"><Edit :size="16"/></button><button @click="deleteActivityItem(item.id)" class="text-red-500 hover:bg-red-100 p-1.5 rounded transition" title="Hapus"><Trash2 :size="16"/></button></div></td></tr><tr v-if="activityList.length === 0"><td :colspan="activityTab === 'meeting' ? 7 : (activityTab === 'quotation' ? 5 : 6)" class="p-12 text-center text-gray-400 italic bg-gray-50/50">Belum ada riwayat aktivitas. Klik tombol "Tambah" di atas.</td></tr></tbody></table></div></div></div></div></Teleport>
    <Teleport to="body"><div v-if="showInputModal" class="fixed inset-0 z-[120] flex items-center justify-center bg-black bg-opacity-70 backdrop-blur-sm p-4"><div class="bg-white rounded-xl shadow-2xl w-full max-w-lg relative animate-fade-in-up"><div class="bg-white p-5 border-b rounded-t-xl flex justify-between items-center"><h3 class="font-bold text-gray-800 text-lg flex items-center gap-2"><span class="w-2 h-6 bg-green-500 rounded-full inline-block"></span> {{ isEditingActivity ? 'Edit Data' : 'Input Data Baru' }}</h3><button @click="showInputModal = false" class="text-gray-400 hover:text-gray-600"><X :size="24"/></button></div><div class="p-6 max-h-[70vh] overflow-y-auto"><form @submit.prevent="saveActivityLog" class="space-y-4"><div class="grid grid-cols-2 gap-4"><div><label class="label">PIC GP</label><select v-model="formActivity.pic_gp" class="input"><option v-for="pic in picList" :key="pic" :value="pic">{{ pic }}</option></select></div><div v-if="activityTab !== 'quotation'"><label class="label">PIC Lead</label><input v-model="formActivity.pic_lead" type="text" class="input"></div></div><div class="grid grid-cols-2 gap-4"><div><label class="label">Date</label><input v-model="formActivity.date" type="date" class="input"></div><div v-if="activityTab !== 'quotation'"><label class="label">Objective</label><input v-model="formActivity.objective" type="text" class="input"></div></div><div v-if="activityTab !== 'quotation'" class="grid grid-cols-2 gap-4"><div><label class="label">Start Time</label><input v-model="formActivity.start_time" type="time" class="input"></div><div><label class="label">End Time</label><input v-model="formActivity.end_time" type="time" class="input"></div></div><div v-if="activityTab === 'followup'" class="space-y-4"><div><label class="label">Follow Up Type</label><select v-model="formActivity.fu_type" class="input"><option>Phone Calls/Telephone</option><option>Texting/Chat</option><option>Follow up leads</option></select></div><div><label class="label">Notes</label><textarea v-model="formActivity.notes" rows="4" class="input" placeholder="Catatan hasil..."></textarea></div></div><div v-if="activityTab === 'meeting'" class="space-y-4"><div><label class="label">Meeting Type</label><select v-model="formActivity.meeting_type" class="input"><option>Visit Meeting</option><option>Online Meeting</option><option>Office Visit Meeting</option></select></div><div v-if="formActivity.meeting_type === 'Visit Meeting'" class="bg-gray-50 p-3 rounded border space-y-3"><div><label class="label">Location</label><input v-model="formActivity.location" type="text" class="input" placeholder="Nama Lokasi / Alamat"></div><div class="grid grid-cols-2 gap-3"><div><label class="label">Latitude</label><input v-model="formActivity.latitude" type="text" class="input"></div><div><label class="label">Longitude</label><input v-model="formActivity.longitude" type="text" class="input"></div></div></div><div><label class="label">MOM</label><textarea v-model="formActivity.mom" rows="4" class="input" placeholder="Minutes of Meeting..."></textarea></div></div><div v-if="activityTab === 'quotation'" class="space-y-4"><div><label class="label">Link Quotation</label><div class="relative"><input v-model="formActivity.link_quotation" type="text" class="input" placeholder="google.com" @blur="autoFixUrl(formActivity, 'link_quotation')" required></div></div><div class="flex items-center gap-2 cursor-pointer pt-2" @click="formActivity.is_send = !formActivity.is_send"><CheckSquare :size="20" class="text-green-600" v-if="formActivity.is_send"/><Square :size="20" class="text-gray-400" v-else/><span class="text-sm font-medium text-gray-700">Sudah Terkirim?</span></div></div><div class="pt-4 flex justify-end gap-3 border-t mt-4"><button type="button" @click="showInputModal = false" class="px-5 py-2.5 rounded-lg border bg-white hover:bg-gray-50 font-medium text-gray-700">Batal</button><button type="submit" class="px-5 py-2.5 rounded-lg bg-green-600 text-white hover:bg-green-700 font-bold shadow-lg flex items-center gap-2"><Save :size="18"/> Simpan</button></div></form></div></div></div></Teleport>
