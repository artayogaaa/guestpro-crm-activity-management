...
const inboundSources = ['Website', 'Referral', 'Reff', 'Reff No Commission', 'Social Media'];
...
// FORM DEAL (UPDATED: pic_lead init null)
const formDeal = ref({
  lead: null,
  deal_type: 'New Deal',
  room: 0,
  pic_lead: null, // <--- Nanti diisi ID (Integer) dari dropdown
  notes: '',
  details: [] 
});
...
// =======================================================================
// API & LOGIC: ADD/EDIT LEAD PROPERTY
// =======================================================================
...
const saveDeal = async () => {
  try {
    ...
    // 1. Update Status Lead -> 'deals'
    await api.patch(`leads/${pk}/`, { status_kanban: 'deals' });
    
    // 2. Simpan Data Deal (Nested Header + Detail)
    ...
    showToast('error', 'Gagal simpan deal'); 
    console.error(error);
  }
};
...
const openManualQuotation = (lead) => {
  draggedItem.value = lead;
  openAddNewTaskModal(lead, 'quotation', 'Quotation');
};
...
const saveDragForm = async () => {
  // VALIDASI MANUAL: Link Quotation
  if (activeDragTab.value === 'quotation' && !formActivity.value.link_quotation) {
    showToast('warning', 'Link Quotation wajib diisi!');
    return;
  }

  try {
    const pk = draggedItem.value.lead_id;
    if (modalMode.value === 'drag') {
        await api.patch(`leads/${pk}/`, { status_kanban: 'follow_up' });
    }
    
    if (activeDragTab.value === 'quotation') {
        await api.post('quotations/', {
            lead: pk,
            pic_gp: formActivity.value.pic_gp,
            date: formActivity.value.date,
            link_quotation: formActivity.value.link_quotation,
            is_send: formActivity.value.is_send
        });
        showToast('success', 'Quotation Disimpan');
    } else {
        const endpoint = activeDragTab.value === 'followup' ? 'followups/' : 'meetings/';
        await api.post(endpoint, { ...formActivity.value, lead: pk });
...
const saveQuotationInitial = async () => {
  if (!formQuotationInitial.value.link_quotation) {
      showToast('warning', 'Link Quotation wajib diisi!');
      return;
  }
  try {
...
const updateStatus = async (evt, newStatus) => {
  if (evt.added) {
    ...
    try { await api.patch(`leads/${item.lead_id}/`, { status_kanban: newStatus }); } 
    catch (error) { showToast('error', 'Gagal update status'); }
  }
};
...
const fetchActivitiesByLead = async (leadId, type) => {
  try {
    let endpoint = 'followups/';
    if (type === 'meeting') endpoint = 'meetings/';
    if (type === 'quotation') endpoint = 'quotations/';

    const response = await api.get(endpoint);
    activityList.value = response.data
      .filter(item => item.lead === leadId)
      .map(item => {
          if (type === 'quotation') return { ...item, id: item.quotation_id };
          return item;
      });
  } catch (error) { showToast('error', 'Gagal memuat riwayat'); }
};

const openActivityModal = async (lead, defaultTab = 'followup') => {
  selectedActivityLead.value = lead;
  showActivityModal.value = true;
  activityTab.value = defaultTab;
  showPicList.value = false; 
  await fetchActivitiesByLead(lead.lead_id, defaultTab);
};

const switchActivityTab = async (tab) => {
  activityTab.value = tab;
  if(selectedActivityLead.value) await fetchActivitiesByLead(selectedActivityLead.value.lead_id, tab);
};

const openInputModal = (item = null) => {
  if (item) {
    isEditingActivity.value = true;
    formActivity.value = JSON.parse(JSON.stringify(item));
  } else {
    isEditingActivity.value = false;
    resetFormActivity(selectedActivityLead.value);
    const statusMap = { 'follow_up': 'Follow Up', 'quotation': 'Quotation' };
    formActivity.value.stage = statusMap[selectedActivityLead.value.status_kanban] || 'Activity Log';
  }
  showInputModal.value = true;
};

const resetFormActivity = (leadObj = null) => {
  formActivity.value = {
    id: null,
    lead: leadObj ? leadObj.lead_id : null,
    pic_gp: leadObj ? leadObj.gp_pic : '', 
    pic_lead: '', 
    date: new Date().toISOString().split('T')[0],
    start_time: '', end_time: '', objective: 'Sales & Leads',
    stage: 'Follow Up',
    fu_type: 'Phone Calls/Telephone', notes: '',
    meeting_type: 'Visit Meeting', location: '', latitude: '', longitude: '', mom: '',
    link_quotation: '', is_send: false
...
const saveActivityLog = async () => {
  // VALIDASI MANUAL: Link Quotation (Activity Log)
  if (activityTab.value === 'quotation' && !formActivity.value.link_quotation) {
     showToast('warning', 'Link Quotation wajib diisi!');
     return;
  }
  ...
  try {
    let endpoint = 'followups/';
    if (activityTab.value === 'meeting') endpoint = 'meetings/';
    if (activityTab.value === 'quotation') endpoint = 'quotations/';

    const payload = { ...formActivity.value, lead: selectedActivityLead.value.lead_id };
    
    if (isEditingActivity.value) {
      await api.put(`${endpoint}${formActivity.value.id}/`, payload);
    } else {
    ...
    fetchActivitiesByLead(selectedActivityLead.value.lead_id, activityTab.value);
  } catch (error) { showToast('error', 'Gagal simpan'); }
};
    ...
    <div class="flex gap-4 overflow-x-auto pb-8 h-[calc(100vh-150px)] items-start">
      ...
      <div class="min-w-[320px] bg-gray-100 rounded-xl p-3 flex flex-col gap-3 h-full">
        ...
        <div class="bg-blue-50/50 p-2 rounded-lg border border-blue-100 flex-1 flex flex-col">
          ...
          <draggable v-model="leadsGenInbound" group="leads" item-key="lead_id" class="flex-1 flex flex-col gap-2 min-h-[50px]">
            <template #item="{element}">
              <div @dblclick="openDetailModal(element)" class="bg-white p-3 rounded shadow-sm border border-gray-200 cursor-move hover:shadow-md transition relative group select-none">
                <div class="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-white/90 rounded p-0.5 shadow z-10">
                   <button @click.stop="openModal(element)" class="text-blue-500 hover:bg-blue-100 p-1 rounded"><Edit :size="14" /></button>
                   <button @click.stop="deleteLead(element.lead_id)" class="text-red-500 hover:bg-red-100 p-1 rounded"><Trash2 :size="14" /></button>
                </div>
                <div class="font-bold text-gray-800 text-sm mb-1 pr-12">{{ element.property }}</div>
                <div class="text-xs text-blue-500 mb-2">{{ element.source }}</div>
                <div class="text-xs text-gray-500 flex items-center gap-1"><User :size="12"/> {{ element.gp_pic }}</div>
              </div>
            </template>
          </draggable>
        </div>
        ...
        <div class="bg-orange-50/50 p-2 rounded-lg border border-orange-100 flex-1 flex flex-col mt-2">
          ...
          <draggable v-model="leadsGenOutbound" group="leads" item-key="lead_id" class="flex-1 flex flex-col gap-2 min-h-[50px]">
            <template #item="{element}">
              <div @dblclick="openDetailModal(element)" class="bg-white p-3 rounded shadow-sm border border-gray-200 cursor-move hover:shadow-md transition relative group select-none">
                <div class="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-white/90 rounded p-0.5 shadow z-10">
                   <button @click.stop="openModal(element)" class="text-blue-500 hover:bg-blue-100 p-1 rounded"><Edit :size="14" /></button>
                ...
                <div class="font-bold text-gray-800 text-sm mb-1 pr-12">{{ element.property }}</div>
                <div class="text-xs text-orange-500 mb-2">{{ element.source }}</div>
                <div class="text-xs text-gray-500 flex items-center gap-1"><User :size="12"/> {{ element.gp_pic }}</div>
              </div>
            </template>
          </draggable>
      ...
      <div class="min-w-[300px] bg-gray-100 rounded-xl p-3 flex flex-col h-full"><h3 class="font-bold text-gray-700 px-2 mb-3 flex justify-between">Deals <span class="bg-green-200 px-2 rounded-full text-xs py-1">{{ leadsDeals.length }}</span></h3>
      <draggable v-model="leadsDeals" group="leads" item-key="lead_id" class="flex-1 flex flex-col gap-2 min-h-[100px]" @change="onChangeDeals">
        <template #item="{element}">
          <div class="bg-white p-3 rounded shadow-sm border-l-4 border-green-500 cursor-move hover:shadow-md transition relative group select-none">
            <div class="font-bold text-gray-800 text-sm mb-1">{{ element.property }}</div>
            <div class="text-xs text-gray-500 mb-2">{{ element.source }}</div>
            <div class="text-xs text-gray-400 flex items-center gap-1"><User :size="10"/> {{ element.gp_pic }}</div>
          </div>
        </template>
      </draggable>
          ...
          <form @submit.prevent="saveDragForm" class="flex flex-col h-full">
              ...
              <div class="px-6 pt-4 pb-0">
                <div class="flex gap-2 bg-gray-100 p-1 rounded-lg overflow-x-auto">
                    <button type="button" @click="activeDragTab = 'followup'" class="flex-1 py-2 rounded-md text-xs font-bold transition-all whitespace-nowrap" :class="activeDragTab === 'followup' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:bg-gray-200'">1. Form Follow Up</button>
                    <button type="button" @click="activeDragTab = 'meeting'" class="flex-1 py-2 rounded-md text-xs font-bold transition-all whitespace-nowrap" :class="activeDragTab === 'meeting' ? 'bg-white text-orange-600 shadow-sm' : 'text-gray-500 hover:bg-gray-200'">2. Form Meeting</button>
                    <button type="button" @click="activeDragTab = 'quotation'" class="flex-1 py-2 rounded-md text-xs font-bold transition-all whitespace-nowrap" :class="activeDragTab === 'quotation' ? 'bg-white text-purple-600 shadow-sm' : 'text-gray-500 hover:bg-gray-200'">3. Form Quotation</button>
                </div>
              </div>
